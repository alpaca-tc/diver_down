...
import { ModuleDependency, buildTopModuleMap } from './buildTopModuleMap'
...
type TopModuleTrProps = {
  module: Module
  topModule: ModuleDependency
  graphOptions: GraphOptions
}

const TopModuleTr: FC<TopModuleTrProps> = ({ module, topModule, graphOptions }) => {
  const linkGraphOptions: GraphOptions = Object.assign({}, graphOptions, {
    modulesList: [...new Set([...topModule.dependencyModules, ...topModule.reverseDependencyModules])].map((mod) => [mod]),
    onlyModule: false,
  ...
  return (
    <tr>
      ...
      <Td>{topModule.sourcesCount}</Td>
      <Td>
        <Link to={`${path.globalDefinition.show()}?${stringify(linkGraphOptions)}`}>
          {topModule.dependencyModules.size} / {topModule.reverseDependencyModules.size}
        </Link>
      </Td>
    </tr>
  )
...
export const GlobalDefinitionModulesTable: FC<GlobalDefinitionModulesProps> = ({ combinedDefinition, graphOptions }) => {
  ...
  const topModuleMap = useMemo(() => buildTopModuleMap(combinedDefinition?.sources || []), [combinedDefinition])
  const sortedTopModuleKeys: Module[] = useMemo(() => {
    const asc = [...topModuleMap.keys()].sort(ascString)
    return sortState === 'asc' ? asc : asc.reverse()
  }, [topModuleMap, sortState])
            ...
            {!combinedDefinition ? (
              ...
              <tbody>
                {sortedTopModuleKeys.map((key) => (
                  <TopModuleTr key={key} module={key} topModule={topModuleMap.get(key)!} graphOptions={graphOptions} />
                ))}
              </tbody>
            )}
          </Table>
