...
import { EmptyTableBody, Table, Th, Text, Td, Button } from '@/components/ui'
...
const SourceTr: FC<{ source: SpecificModuleSource }> = ({ source }) => {
  ...
  const dependencies = useMemo(() => {
    return source.dependencies.toSorted(
      (a, b) => ascString(String(a.module), String(b.module)) || ascString(String(a.sourceName), String(b.sourceName)),
    )
  }, [source])
  ...
  return (
    <>
      <tr>
        ...
        <Td>{source.dependencies.length}</Td>
        <Td></Td>
        <Td></Td>
      </tr>
...
export const SourcesContent: FC<{ sources: SpecificModule['sources'] }> = ({ sources }) => {
  ...
  const sortedSources = useMemo(() => {
    return sortSources(sources, sort.key, sort.sort)
  }, [sort, sources])
      ...
      {sortedSources.length === 0 ? (
        ...
        <tbody>
          {sortedSources.map((source) => (
            <SourceTr key={source.sourceName} source={source} />
          ))}
        </tbody>
      )}
    </Table>
